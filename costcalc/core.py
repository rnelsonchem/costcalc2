...
class CoreCost(object):
    ...
    def _column_clear(self, excel=False):
        ...
        # For dynamic Excel
        if excel:
            excel_cols = [dyn_cst, dyn_rkg, dyn_rrmc, 
                        dyn_rrmp, dyn_pkg, 
                        dyn_prmc, dyn_prmp,
                        ]
    ...
    def _rxn_cost(self, prod, step, amp=1.0, eamp='', excel=False):
        ...
        if excel:
            ...
            cells = [f'{ecols[rxn_rmc]}{r}' for r in\
                    data.loc[mask, 'rnum']]
            rs = ','.join(cells)
        ...
        # And for Excel
        if excel:
            data[dyn_pkg] = '=' + ecols[rxn_kg] + data['rnum']\
                    + eamp
    ...
    def excel(self, ):
        ...
        d_cols = [dyn_cst, rxn_kg, rxn_rmc, rxn_rmp, 
                  prd_kg, prd_rmc, prd_rmp,
                  'rnum']
